...
export function SalesReportHub() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(apiUrl(`/api/orders/pos-sales?${params}`));
  ...
  const openDetail = (row: SaleRow) => {
    setDetailRow(row);
    detailModal.open();
  };

  const openPrint = (row: SaleRow) => {
    ...
    <div className="mx-auto flex w-full max-w-[1400px] flex-col gap-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-2">
          <Boxes3 width={24} height={24} />
          <div>
            <h1 className="text-xl font-bold tracking-tight">Ventas</h1>
            <p className="text-sm text-muted">
              Reporte de facturación / ventas de caja por día.
            </p>
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            size="sm"
            variant="primary"
            onPress={() => router.push(appRoutes.operation.cash)}
          >
            Ir a caja
          </Button>
        </div>
      ...
      <section className="overflow-hidden rounded-2xl border border-separator bg-surface">
        <div className="border-b border-separator px-3 py-2">
          <h2 className="text-sm font-bold">Facturación × día</h2>
        </div>
        {loading ? (
                ...
                {data.rows.map((row) => (
                  ...
                  >
                    ...
                    <td className="max-w-[7rem] truncate px-2 py-1.5">
                      {row.sellerLabel || row.sellerName || "—"}
                    </td>
